...
## Workspace visual polish — 2026-09-05

The four workspace sections now share a sidebar with icon tiles, short descriptions, a distinct active border and contextual breadcrumbs. Wallet Cleanup stays selected on its read-only route; Docs stays selected in its guides; safety routes belong to the diagnostics section. Hover is visually different from the active state.

Cleanup has clearer selected rows, a framed TON reserve and a contrasting review panel. Swap Doctor has a guided empty state, connected event markers, a recorded-event count and a report panel; no sample events are invented for an empty session. Developer Kit moves from the App route into a component with four capability cards, an exact-amount source example, developer-guide/source links, a safety-lab card and explicit live-use limitations. Docs gains an icon menu, audience shortcuts, a framed section index and pagination cards. Shared styles are scoped to the workspace; the public landing retains its design.

Validation: `pnpm check` passed with 181 tests, typecheck, production build, standalone API checks, lint and formatting. All 51 browser tests passed. New coverage includes Developer Kit, empty Doctor and Docs introduction accessibility at 360/1440px, guide/lab links, and one active section across nested/live routes. Existing suites continue to cover cleanup, populated timelines, paused queues, local report downloads, safety recovery, Docs anchors and the landing page. Visual review covered all four desktop sections at 1440px, mobile Docs and Developer Kit, and a populated mobile Doctor timeline at 390px. No console errors or layout blockers were found. No transaction or provider behavior changed.
